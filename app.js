...
// First Match Timer
const firstMatch = new Date('2025-11-17T16:00:00+05:45'); // 4:00 PM NPT
function updateTimer() {
  const now = new Date();
  const diff = firstMatch - now;
  if (diff < 0) {
    els.timer.textContent = 'Match Started';
...
els.googleSigninBtn.onclick = () => {
  if (location.protocol === 'http:' && location.hostname !== 'localhost') {
    els.authError.textContent = 'HTTPS required. Deploy to Vercel or use ngrok.';
    return;
  }
...
// Filter & Render Players
function filterPlayers() {
  let players = nplPlayers
    ...
    return `
      <div class="player ${isSelected ? 'selected' : ''}" data-name="${player.name}">
        ...
        <div class="player-info">
          ...
          <div class="player-team">${player.team} • Selected by 45%</div>
        </div>
        <div class="player-stats">
          ...
          <div>
            ...
            <div class="stat-value">${player.credits}</div>
          </div>
        </div>
        <button class="add-btn ${isSelected ? 'selected' : ''}">${isSelected ? 'Check' : 'Plus'}</button>
      </div>
    `;
  }).join('');
...
function updateStats() {
  ...
  const canSave = selected.length === 11 && captain && viceCaptain && captain !== viceCaptain;
...
function renderPreview() {
  const groups = ['WK', 'BAT', 'AR', 'BOWL'].map(role => {
    ...
    return players ? `<div class="role-group"><div class="role-header">${role} (${selected.filter(p=>getPlayer(p).role===role).length})</div>${players}</div>` : '';
  }).join('');
...
els.saveTeamBtn.onclick = () => {
  if (!currentUser || !captain || !viceCaptain || captain === viceCaptain) return;
...
function loadUserTeam() {
  ...
  db.collection('teams').doc(currentUser.uid).get()
    ...
    .catch(err => console.warn('Load failed (offline?):', err));
}
